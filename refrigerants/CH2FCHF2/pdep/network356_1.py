...
"""),
    E0 = (383.474,'kJ/mol'),
    modes = [
        HarmonicOscillator(frequencies=([1380,1390,370,380,2900,435,2950,1000,145,326,398,834,1303,180,180,180,180,501.185,1531.91,1531.95,1531.95,1531.98,1532.05],'cm^-1')),
        HinderedRotor(inertia=(0.187858,'amu*angstrom^2'), symmetry=1, barrier=(4.31922,'kJ/mol'), semiclassical=False),
    ],
    spinMultiplicity = 1,
    opticalIsomers = 1,
...
"""),
    ...
    molecularWeight = (116.065,'amu'),
    energyTransferModel = SingleExponentialDown(alpha0=(3.5886,'kJ/mol'), T0=(300,'K'), n=0.85),
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[1.47724,0.0646367,-0.000114488,1.12234e-07,-4.25576e-11,26456.5,19.9527], Tmin=(100,'K'), Tmax=(811.121,'K')), NASAPolynomial(coeffs=[4.65288,0.0303041,-1.64669e-05,3.28874e-09,-2.31786e-13,26555.6,9.08311], Tmin=(811.121,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(219.291,'kJ/mol'), Cp0=(33.2579,'J/(mol*K)'), CpInf=(228.648,'J/(mol*K)'), comment="""Thermo group additivity estimation: group(O2s-CsCs) + group(Cds-CdsCsCs) + group(Cds-Cds(Cds-Cds)H) + group(CdCddCF) + group(CJ2_singlet-FO) + group(Cdd-CdsCds) + ring(cyclobutadiene_13)"""),
)

species(
    label = 'FC1=C=C[C-][O+]=C1F(3041)',
    structure = adjacencyList("""1  F u0 p3 c0 {5,S}
2  F u0 p3 c0 {7,S}
...
"""),
    ...
    molecularWeight = (116.065,'amu'),
    energyTransferModel = SingleExponentialDown(alpha0=(3.5886,'kJ/mol'), T0=(300,'K'), n=0.85),
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[1.22162,0.0736691,-0.000135897,1.37618e-07,-5.27365e-11,11748.9,41.3319], Tmin=(100,'K'), Tmax=(832.957,'K')), NASAPolynomial(coeffs=[2.67139,0.0381323,-2.04444e-05,4.02955e-09,-2.8091e-13,12498.7,40.553], Tmin=(832.957,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(96.9557,'kJ/mol'), Cp0=(33.2579,'J/(mol*K)'), CpInf=(232.805,'J/(mol*K)'), comment="""Thermo group additivity estimation: group(O2s-CsCs) + group(CsJ2_singlet-CsH) + group(COCFO) + group(Cds-CdsCsH) + group(CdCddCF) + group(Cdd-CdsCds) + ring(124cyclohexatriene)"""),
)

species(
    label = 'FC1=C=CC(F)=[O+][C-]1(3042)',
    structure = adjacencyList("""1  F u0 p3 c0 {5,S}
2  F u0 p3 c0 {7,S}
3  O u0 p1 c+1 {4,S} {5,D}
...
"""),
    ...
    molecularWeight = (116.065,'amu'),
    energyTransferModel = SingleExponentialDown(alpha0=(3.5886,'kJ/mol'), T0=(300,'K'), n=0.85),
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[1.1465,0.0721718,-0.000122994,1.1759e-07,-4.38168e-11,10848.5,40.9922], Tmin=(100,'K'), Tmax=(815.957,'K')), NASAPolynomial(coeffs=[5.17979,0.0336724,-1.7793e-05,3.50826e-09,-2.45554e-13,10813.7,26.1744], Tmin=(815.957,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(89.4208,'kJ/mol'), Cp0=(33.2579,'J/(mol*K)'), CpInf=(232.805,'J/(mol*K)'), comment="""Thermo group additivity estimation: group(O2s-CsCs) + group(CsJ2_singlet-CsH) + group(COCFO) + group(Cds-Cds(Cds-Cds)H) + group(CdCddCF) + group(Cdd-CdsCds) + ring(124cyclohexatriene)"""),
)

species(
    label = '[C-]=[O+]C1=CC(F)=C1F(3043)',
...
reaction(
    label = 'reaction1',
    reactants = ['[C-]=[O+]C1(F)C=C=C1F(2036)'],
...
reaction(
    label = 'reaction4',
    reactants = ['[C-]=[O+]C1(F)C=C=C1F(2036)'],
    products = ['FC1=C=CC(F)=[O+][C-]1(3042)'],
    transitionState = 'TS4',
...
reaction(
    label = 'reaction6',
    reactants = ['F2(78)', '[C-]=[O+]C1=C=C=C1(3044)'],
    products = ['[C-]=[O+]C1(F)C=C=C1F(2036)'],
    transitionState = 'TS6',
