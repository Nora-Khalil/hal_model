...
gas = ct.Solution('/scratch/westgroup/David/halogen_models/rmg_combustion_paper_FFCM/suppressants/2-BTP/cantera/chem_annotated.cti')
...
for i in  range(len(BTPmole_list)):
    try: 
        ...
        #vol_frac_dict = {'CH4': (x/norm_ox), 'O2':((1-x)*.21)/norm_ox, 'N2':((1-x)*0.79)/norm_ox}
        vol_frac_dict = {'CH4(5)': 0.5, 'O2(2)': 1, 'N2': 3.76, '2-BTP(1)': x }
        ...
        print(vol_frac_dict)
        print(f"O2/CH4 ratio = {vol_frac_dict['O2(2)']/vol_frac_dict['CH4(5)']}. Complete combustion takes 2")
...
with open('final_calcs_Davidsfromscratch_2BTP_range_10pts.csv', 'w+') as g:
