...
os.command = f'scp chem0145.inp copies/copy_chem0145.inp'
...
#this is a string of the output when I try to convert this file to a cti file. Will probably produce an error
    output = getoutput( f'ck2cti --input=copy_chem0145.inp --transport=tran0145.dat') 
    ...
    if re.search('PASSED',output):
        ...
        if re.search('Encountered\sunmarked\sduplicate\sreaction',output):
            ...
            with open('copy_chem0145.inp','r') as f:
                data = f.readlines()
            ...
            with open(f'copy_chem0145.inp','w+') as f: 
                for l in data: 
                    f.write(l)
                    x==0
