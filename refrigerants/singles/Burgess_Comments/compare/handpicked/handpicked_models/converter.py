...
string = 'chem_1285.inp  chem_1321_1285.inp  chem_1321_1322_1285.inp  chem_1321_1322.inp  chem_1321.inp  chem_1322_1285.inp  chem_1322.inp  chem_.inp'

# list_of_files = [file for file in os.listdir('.') if re.search('chem_[[0-9]+]?.inp', file)]

list_of_files = string.split('  ')

print(list_of_files)
                           
for file in list_of_files: 
        match = re.search('chem_([0-9]+).inp',file)
        match2 =re.search('chem_([0-9]+)_([0-9]+).inp',file)
        match3 = re.search('chem_([0-9]+)_([0-9]+)_([0-9]+).inp',file)
        ...
        if match: 
            os.system(f'scp tran.dat copies/tran_{match.group(1)}.dat')
            transport_file = f'tran_{match.group(1)}.dat'
        if match2: 
            os.system(f'scp tran.dat copies/tran_{match2.group(1)}_{match2.group(2)}.dat')
            transport_file = f'tran_{match2.group(1)}_{match2.group(2)}.dat'
        if match3: 
            os.system(f'scp tran.dat copies/tran_{match3.group(1)}_{match3.group(2)}_{match3.group(3)}.dat')
            transport_file = f'tran_{match3.group(1)}_{match3.group(2)}_{match3.group(3)}.dat'
        ...
        #this is a string of the output when I try to convert this file to a cti file. Will probably produce an error
            output = getoutput( f'ck2cti --input=copy_{file} --transport={transport_file}') 
