...
list_of_blends = ['chem0259.inp']  ### dont forget to change the _168_species in this script. control f it

fullpath = '/work/westgroup/nora/Code/projects/halogens/refrigerants/blends/Papas_Zhang_blends/rmg_built_blends_with_species_in_core/2,3,3,3-tetrafluoropropene_propane/chemkin/'


file_name = 'chem0259.inp'
...
def convert(full_path_to_chemkin_folder):
    ...
    command = f'scp {file_name} copies/copy_168_species_{file_name}'
    os.system(command)
    ...
    #this is a string of the output when I try to convert this file to a cti file. Will probably produce an error
        output = getoutput( f'ck2cti --input=copy_168_species_{file_name} --transport=tran.dat') 
        ...
        if re.search('PASSED',output):
            ...
            if re.search('Encountered\sunmarked\sduplicate\sreaction',output):
                ...
                with open(f'./copy_168_species_{file_name}','r') as f:
                    data = f.readlines()
                ...
                with open(f'copy_168_species_{file_name}','w+') as f: 
                    for l in data: 
                        f.write(l)
                        x==0
