...
def convert(file_name):
    ...
    os.command = f'scp {file_name} copies/copy_2_{file_name}'
    ...
    #this is a string of the output when I try to convert this file to a cti file. Will probably produce an error
        output = getoutput( f'ck2cti --input=copy_2_{file_name} --transport=tran.dat') 
        ...
        if re.search('PASSED',output):
            ...
            if re.search('Encountered\sunmarked\sduplicate\sreaction',output):
                ...
                with open(f'copy_2_{file_name}','r') as f:
                    data = f.readlines()
                ...
                with open(f'copy_2_{file_name}','w+') as f: 
                    for l in data: 
                        f.write(l)
                        x==0
