...
directory = '/work/westgroup/nora/Code/projects/halogens/refrigerants/blends/blends_of_two_equal_parts_species_in_core_redo/CH2F2_CH3F/chemkin/copies/copy_cross_rxns_deleted_updated.cti'
gas = ct.Solution(directory)
blend_name = 'CH2F2_CH3F'
...
for i in  range(len(vol_frac_list)):
    try:
        ...
        vol_frac_dict = {'CH2F2(1)': (x/2/norm_ox), 'CH3F(22)': (x/2/norm_ox), 'O2(2)':((1-x)*.21)/norm_ox, 'N2':((1-x)*0.79)/norm_ox}
        ...
        try:
            if i!=0:
                d = f'data/_deleted_updated_{vol_frac_list[i-1]}.csv'
                if os.path.exists(d):  
        ...
        df1 = sltn.to_pandas()
        df1.to_csv(f'data/_deleted_updated_{x}.csv', index=False)            
...
with open('blend_flamespeeds_deleted_updated.csv', 'w+') as g:
