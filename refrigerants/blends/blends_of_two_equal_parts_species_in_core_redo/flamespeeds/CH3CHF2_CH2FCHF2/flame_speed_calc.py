...
directory = '/work/westgroup/nora/Code/projects/halogens/refrigerants/blends/blends_of_two_equal_parts_species_in_core_redo/CH3CHF2_CH2FCHF2/chemkin/copies/copy_chem.cti'
gas = ct.Solution(directory)
blend_name = 'CH3CHF2_CH2FCHF2'
...
for i in  range(len(vol_frac_list)):
    try:
        ...
        vol_frac_dict = {'CH3CHF2(1)': (x/2/norm_ox), 'S(180)': (x/2/norm_ox), 'O2(2)':((1-x)*.21)/norm_ox, 'N2':((1-x)*0.79)/norm_ox}
        ...
        try:
            if i!=0:
                d = f'data/_2_{vol_frac_list[i-1]}.csv'
                if os.path.exists(d):  
        ...
        df1 = sltn.to_pandas()
        df1.to_csv(f'data/_2_{x}.csv', index=False)            
...
with open('blend_flamespeeds_2_.csv', 'w+') as g:
